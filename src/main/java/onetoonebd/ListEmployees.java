package onetoonebd;
import java.util.List;

import org.hibernate.Query;
import org.hibernate.Session;
import org.hibernate.SessionFactory;
import org.hibernate.cfg.Configuration;


public class ListEmployees {

	public static void main(String[] args) throws Exception {
		Configuration c = new Configuration();
		c.configure(); // loads hibernate.cfg.xml
		c.addResource("onetoonebd/one_to_one_bd.hbm.xml");

		SessionFactory sf = c.buildSessionFactory();
		Session s = sf.openSession();
		
		/*
		// employees 
		Query q = s.createQuery("from Employee e  join fetch  e.membership");

		for (Employee e : (List<Employee>) q.list()) {
			System.out.println( e.getName() + " , " +  e.getMembership().getType());
		}

		
		System.out.println("Done");
		
		*/
		
		// Memberships

		Query q2 = s.createQuery("from Membership");

		for (Membership m : (List<Membership>) q2.list()) {
			System.out.println( m.getType() + "," + m.getEmployee().getName());
		}

		s.close();
		sf.close();
	}

}
